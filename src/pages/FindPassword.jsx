...
function FindPassword() {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      setEmailMessage(getErrorMessage(error, '인증번호 발송에 실패했습니다.'));
      setEmailMessageType('error');
    } finally {
  ...
  const verifyCodeAutomatically = async (nextCode) => {
    ...
    if (!trimmedEmail || !isCodeSent || nextCode.length !== 6) {
      return;
    }
    ...
    try {
      ...
      setIsCodeVerified(false);
      setCodeMessage(
        getErrorMessage(error, '인증번호가 일치하지 않거나 만료되었습니다.'),
      );
  ...
  const isSubmitDisabled =
    !isCodeVerified || newPassword.length < 8 || isResettingPassword;
    ...
    <main className="find-password-page">
      ...
      <div className="find-password-form">
        ...
        <div className="find-password-field">
          ...
          {passwordMessage && (
            <p className="find-password-message error">{passwordMessage}</p>
          )}
        </div>
      </div>
